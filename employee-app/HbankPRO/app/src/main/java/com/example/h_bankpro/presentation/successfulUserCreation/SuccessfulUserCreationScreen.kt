...
@Composable
fun SuccessfulUserCreationScreen(
    navController: NavController,
    viewModel: SuccessfulUserCreationViewModel = koinViewModel()
) {
    LaunchedEffect(key1 = true) {
        viewModel.navigationEvent.collect { event ->
            when (event) {
                SuccessfulUserCreationNavigationEvent.NavigateToMain ->
    ...
    Column(
        ...
        SuccessMessage(textRes = R.string.successful_user_creation)
        Spacer(modifier = Modifier.weight(1f))
        CustomButton(
