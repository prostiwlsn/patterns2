...
import com.example.h_bankpro.presentation.successfulRateCreation.SuccessfulRateCreationNavigationEvent
import com.example.h_bankpro.presentation.successfulRateCreation.SuccessfulRateCreationViewModel
import org.koin.androidx.compose.koinViewModel

@Composable
fun SuccessfulRateEditingScreen(
    navController: NavController,
    viewModel: SuccessfulRateCreationViewModel = koinViewModel()
) {
    LaunchedEffect(key1 = true) {
        viewModel.navigationEvent.collect { event ->
            when (event) {
                SuccessfulRateCreationNavigationEvent.NavigateToMain ->
    ...
    Column(
        ...
        SuccessMessage(textRes = R.string.successful_rate_editing)
        Spacer(modifier = Modifier.weight(1f))
        CustomButton(
